...
1. 将本项目下载到本地
```
git clone https://github.com/TangFengX/MakeVeri.git --recursive
```
2. 确定依赖是否安装
```
make check
...
`nxdclite` 是一种高效的虚拟 FPGA 引脚绑定配置格式。通过引入大括号展开、多重笛卡尔积和循环计算等特性，它能将原本冗长的 `.nxdc` 文件压缩至极短，并提供严谨的硬件冲突校验。

> **注意**：编辑完成后，需执行 `make genbind` 调用转换工具，将 `.nxdclite` 编译为标准的 `.nxdc` 文件方可生效。
...
### 1.1 顶层定义
文件首行定义顶层模块名，转换时该行将被完整保留。
- **示例**: `top=top_module`

### 1.2 标准绑定
格式为 `端口名 (引脚列表)`。引脚之间可以用**空格**或**逗号**分隔。
...
### 2.1 范围推断 (`-`)
在任何支持引脚的地方，使用 `-` 连接数字或字母可实现自动序列扩展（支持正序与反序）。
- **数字**: `SW7-0` $\rightarrow$ `SW7, SW6, SW5, SW4, SW3, SW2, SW1, SW0`
- **字母**: `SEG0A-G` $\rightarrow$ `SEG0A, SEG0B, SEG0C, SEG0D, SEG0E, SEG0F, SEG0G`

### 2.2 大括号展开 (`{}`)
大括号用于对前缀进行批量组合。内部元素**强制要求使用空格分隔**（范围表达式除外）。
- **基本用法**: `LD{15 14 0}` $\rightarrow$ `LD15, LD14, LD0`
- **混合用法**: `LD{15 14-12 0}` $\rightarrow$ `LD15, LD14, LD13, LD12, LD0`

### 2.3 多重笛卡尔积（多括号组合）
支持连续使用多个大括号，工具会自动进行笛卡尔积展开。
- **嵌套逻辑**: `LD{2 1}{A-B}` 
  1. 第一步展开: `LD2{A B}`, `LD1{A B}`
  2. 最终展开: `LD2A, LD2B, LD1A, LD1B`
- **实战示例**: `SEG{7-4}{A-G}` 可一次性生成 4 个数码管的所有段码引脚。

### 2.4 循环迭代器 (`@`) 与 任意计算 (`[@]`)
使用 `@` 批量生成结构相似的行，并在 `[]` 内进行数学运算。
- **语法**: `@开始-结束 表达式`
- **示例**: `@0-7 seg@ (SEG[@+0]{A-G}, DEC@P)`
- **计算支持**: 支持 `+`, `-`, `*`, `/`, `%` 及括号运算。
  - 示例: `VGA_R[@*2+1]` 当 `@=2` 时展开为 `VGA_R5`。

---

## 3. 约束、校验与 Debug

为了保证硬件绑定的绝对安全，转换工具内置了三重校验机制：

1. **合法性检查 (Legality)**: 
   所有生成的物理引脚必须存在于 `pins` 数据库中。若拼写错误或引用了不存在的引脚（如 `LD16`），工具将报错。
2. **冲突检查 (Conflict)**:
   - **引脚冲突**: 禁止将同一个物理引脚（如 `LD0`）分配给多个逻辑端口。
   - **端口冲突**: 禁止重复定义同一个逻辑端口名。
3. **错误高亮定位**:
   当校验失败时，工具会在控制台以 **红底白字** 高亮显示出问题的代码片段，并输出上下文。
4. **调试模式**: 
   运行工具时添加 `--verbose` 或 `-v` 参数，可以查看每一行引脚展开的详细推导过程。
...
top=top
# 基础范围与组合
ledr (LD{15 14-4 3 2 1 0}) 
sw (SW{7-0})
# 多重括号展开
btn (BTN{L U C D R})
# 循环与计算
@0-1 vga_data (VGA_R[@*2], VGA_R[@*2+1])
...
vga_data (VGA_R0, VGA_R1)
vga_data (VGA_R2, VGA_R3)
```
